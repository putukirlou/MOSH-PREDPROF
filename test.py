...
def listmessages(cursor, connection, args):
    ...
    # Получаем все банкоматы
    query_atm = "SELECT * FROM atm;"
    cursor.execute(query_atm)
    atm_data = {atm["device_id"]: atm for atm in cursor.fetchall()}  # Словарь для быстрого поиска
    ...
    for message in messages:
        ...
        if "нужен механик" in event_type or "не работает" in event_type:
            status = 0  # Банкомат не работает
        else:
            status = 1  # Банкомат работает

        # Обновляем статус в БД
        if device_id in atm_data:
            cursor.execute("UPDATE atm SET status = ? WHERE device_id = ?;", (status, device_id))

        # Подсчет для среднего арифметического
        total_status += status
        count_status += 1

    # Считаем средний статус
    average_status = total_status / count_status if count_status > 0 else 0
    args["average_status"] = round(average_status, 2)

    # Подготавливаем данные для рендеринга
    args["messages"] = [dict(msg) for msg in messages]
